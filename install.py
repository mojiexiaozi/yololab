...
TORCH: Dict[str, str] = {"default": "default", "cpu": "cpu"}
...
if platform.system().lower() == "linux" or platform.system().lower() == "windows":
    ...
    ONNXRUNTIMES["openvino"] = ("openvino-onnxruntime", "1.16.0")
...
def cli():
    program = ArgumentParser(
        formatter_class=lambda prog: HelpFormatter(prog, max_help_position=130)
    )
    program.add_argument(
        "--torch",
        help="select the variant of torch to install",
        choices=TORCH.keys(),
    )
    program.add_argument(
        "--onnxruntime",
        help="select the variant of onnxruntime to install",
        choices=ONNXRUNTIMES.keys(),
    )
    program.add_argument(
        "--skip-venv", help="skip the virtual environment check", action="store_true"
    )
    program.add_argument(
        "-v",
        "--version",
        version="lightlab 0.1.0",
        action="version",
    )
    run(program)


def run(program: ArgumentParser):
    ...
    if args.torch and args.onnxruntime:
        answers = {"torch": args.torch, "onnxruntime": args.onnxruntime}
    else:
        answers = inquirer.prompt(
            [
                inquirer.List(
                    "torch",
                    message="select the variant of torch to install",
                    choices=list(TORCH.keys()),
                ),
                inquirer.List(
                    "onnxruntime",
                    message="select the variant of onnxruntime to install",
    ...
    if answers:
        ...
        if torch_wheel == "default":
            ...
            subprocess.call(
                [
                    ...
                    "requirements.txt",
                    "--extra-index-url",
                    "https://download.pytorch.org/whl/" + torch_wheel,
                    "--force-reinstall",
                ]
            )
        ...
        if onnxruntime == "rocm-5.4.2" or onnxruntime == "rocm-5.6":
            ...
            subprocess.call(
                ["pip", "uninstall", "onnxruntime", onnxruntime_name, "-y", "-q"]
            )
            if onnxruntime == "cuda-12.1":
                subprocess.call(
                    [
                        "pip",
                        "install",
                        onnxruntime_name + "==" + onnxruntime_version,
                        "--extra-index-url",
                        "https://pkgs.dev.azure.com/onnxruntime/onnxruntime/_packaging/onnxruntime-cuda-12/pypi/simple",
                        "--force-reinstall",
                    ]
                )
            else:
