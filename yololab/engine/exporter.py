...
from yololab.cfg import get_cfg
from yololab.data.dataset import YOLODataset
from yololab.nn.autobackend import check_class_names, default_class_names
from yololab.nn.modules import C2f, Detect
from yololab.nn.tasks import DetectionModel, SegmentationModel, WorldModel
...
from yololab.utils.checks import (
    PYTHON_VERSION,
    check_imgsz,
    check_is_path_safe,
    check_requirements,
...
from yololab.utils.files import file_size, spaces_in_path
...
class Exporter:
    ...
    def __call__(self, model=None):
        ...
        if isinstance(model, WorldModel):
            LOGGER.warning(
                ...
                "See https://docs.yololab.com/models/yolo-world for details."
            )
        ...
        for m in model.modules():
            if isinstance(m, C2f) and not any(
        ...
        description = (
            f'yololab {self.pretty_name} model {f"trained on {data}" if data else ""}'
        )
        self.metadata = {
    ...
    def export_onnx(self, prefix=colorstr("ONNX:")):
        ...
        if self.args.simplify:
            ...
            if ARM64:
                check_requirements(
                    "cmake"
                )  # 'cmake' is needed to build onnxsim on aarch64
    ...
    def export_openvino(self, prefix=colorstr("OpenVINO:")):
        ...
        if self.args.int8:
            ...
            def transform_fn(data_item):
                ...
                im = (
                    data_item["img"].numpy().astype(np.float32) / 255.0
            ...
            dataset = YOLODataset(
                data["val"], data=data, imgsz=self.imgsz[0], augment=False
            )
            n = len(dataset)
            if n < 300:
                LOGGER.warning(
    ...
    def export_ncnn(self, prefix=colorstr("NCNN:")):
        ...
        if not pnnx.is_file():
            ...
            try:
                ...
                LOGGER.warning(
                    f"{prefix} WARNING ⚠️ PNNX GitHub assets not found: {e}, using default {url}"
                )
    ...
    def export_engine(self, prefix=colorstr("TensorRT:")):
        ...
        f_onnx, _ = (
            self.export_onnx()
        )  # run before trt import https://github.com/yololab/yololab/issues/7016
    ...
    def export_saved_model(self, prefix=colorstr("TensorFlow SavedModel:")):
        ...
        check_version(
            ...
            name="tensorflow",
            verbose=True,
            msg="https://github.com/yololab/yololab/issues/5161",
        )
        import onnx2tf
            ...
            if self.args.data:
                ...
                for i, batch in enumerate(dataset):
                    if i >= 100:  # maximum number of calibration images
                        break
                    im = batch["img"].permute(1, 2, 0)[
                        None
                    ]  # list to nparray, CHW to BHWC
                    images.append(im)
                f.mkdir()
    ...
    def export_edgetpu(self, tflite_model="", prefix=colorstr("Edge TPU:")):
        ...
        LOGGER.warning(
            f"{prefix} WARNING ⚠️ Edge TPU known bug https://github.com/yololab/yololab/issues/1185"
        )
...
class IOSDetectModel(torch.nn.Module):
    """Wrap an yololab YOLO model for Apple iOS CoreML export."""
