...
HELP_MSG = """
    ...
    2. Use the Python SDK:
        ...
        results = model('https://yololab.com/images/bus.jpg')  # predict on an image
        ...
        YOLOv8 'yolo' CLI commands use the following syntax:
            ...
            Where   TASK (optional) is one of [detect, segment, classify]
                    ...
                    ARGS (optional) are any number of custom 'arg=value' pairs like 'imgsz=320' that override defaults.
                        See all ARGS at https://docs.yololab.com/usage/cfg or with 'yolo cfg'
    ...
    Community: https://community.yololab.com
    GitHub: https://github.com/yololab/yololab
    """
...
class TQDM(tqdm_original):
    """
    Custom yololab tqdm class with different default arguments.
    ...
    def __init__(self, *args, **kwargs):
        """
        Initialize custom yololab tqdm class with different default arguments.
...
class SimpleClass:
    """yololab SimpleClass is a base class providing helpful string representation, error reporting, and attribute
...
class IterableSimpleNamespace(SimpleNamespace):
    """yololab IterableSimpleNamespace is an extension class of SimpleNamespace that adds iterable functionality and
    ...
    if WINDOWS and sys.stdout.encoding != "utf-8":
        try:
            ...
            formatter = CustomFormatter(
                "%(message)s"
            )  # Use CustomFormatter to eliminate UTF-8 output as last recourse
...
class ThreadingLocked:
    ...
    Example:
        ```python
        from yololab.utils import ThreadingLocked
...
def get_user_config_dir(sub_dir="yololab"):
    """
...
class TryExcept(contextlib.ContextDecorator):
    """
    yololab TryExcept class. Use as @TryExcept() decorator or 'with TryExcept():' context manager.
...
class SettingsManager(dict):
    """
    Manages yololab settings stored in a YAML file.

    Args:
        file (str | Path): Path to the yololab settings YAML file. Default is USER_CONFIG_DIR / 'settings.yaml'.
    ...
    def __init__(self, file=SETTINGS_YAML, version="0.0.4"):
        ...
        from yololab.utils.torch_utils import torch_distributed_zero_first
        ...
        with torch_distributed_zero_first(RANK):
            ...
            correct_types = all(
                type(a) is type(b)
                for a, b in zip(self.values(), self.defaults.values())
            )
            correct_version = check_version(self["settings_version"], self.version)
            if not (correct_keys and correct_types and correct_version):
                LOGGER.warning(
                    "WARNING ⚠️ yololab settings reset to default values. This may be due to a possible problem "
                    "with your settings or a recent yololab package update. "
...
def deprecation_warn(arg, new_arg, version=None):
    ...
    LOGGER.warning(
        f"WARNING ⚠️ '{arg}' is deprecated and will be removed in 'yololab {version}' in the future. "
