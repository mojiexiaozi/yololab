...
def check_version(
    ...
    if not current:  # if current is '' or None
        ...
        0
    ].isdigit():  # current is package name rather than version string, i.e. current='yololab'
        try:
...
def check_latest_pypi_version(package_name="yololab"):
...
def check_requirements(
    ...
    if s:
        if install and AUTOINSTALL:  # check environment variable
            ...
            LOGGER.info(
                f"{prefix} yololab requirement{'s' * (n > 1)} {pkgs} not found, attempting AutoUpdate..."
            )
            try:
                t = time.time()
...
def check_yolo(verbose=True, device=""):
    ...
    from yololab.utils.torch_utils import select_device
...
def collect_system_info():
    ...
    from yololab.utils import ENVIRONMENT, is_git_dir
    from yololab.utils.torch_utils import get_cpu_info
    ...
    for r in parse_requirements(package="yololab"):
        try:
            current = metadata.version(r.name)
            is_met = (
                "✅ " if check_version(current, str(r.specifier), hard=True) else "❌ "
            )
...
def check_amp(model):
    ...
    try:
        ...
        LOGGER.warning(
            ...
            f"Unable to load YOLOv8n due to possible yololab package modifications. {warning_msg}"
        )
...
def print_args(args: Optional[dict] = None, show_file=True, show_func=False):
    ...
    def strip_auth(v):
        """Clean longer yololab HUB URLs by stripping potential authentication information."""
        return (
            clean_url(v)
