...
def get_save_dir(args, name=None):
    ...
    if getattr(args, "save_dir", None):
        ...
        from yololab.utils.files import increment_path
...
def handle_yolo_hub(args: List[str]) -> None:
    """
    Handle yololab HUB command-line interface (CLI) commands.

    This function processes yololab HUB CLI commands such as login and logout.
    ...
    Example:
        ...
        ```
    """
    from yololab import hub

    if args[0] == "login":
        key = args[1] if len(args) > 1 else ""
        # Log in to yololab HUB using the provided API key
        hub.login(key)
    elif args[0] == "logout":
        # Log out from yololab HUB
        hub.logout()
...
def handle_explorer():
    ...
    subprocess.run(
        [
            "streamlit",
            "run",
            ROOT / "data/explorer/gui/dash.py",
...
def entrypoint(debug=""):
    ...
    for a in merge_equals_args(args):  # merge spaces around '=' sign
        if a.startswith("--"):
            LOGGER.warning(
                f"WARNING ⚠️ '{a}' does not require leading dashes '--', updating to '{a[2:]}'."
            )
            a = a[2:]
        if a.endswith(","):
            LOGGER.warning(
                f"WARNING ⚠️ '{a}' does not require trailing comma ',', updating to '{a[:-1]}'."
            )
            a = a[:-1]
        if "=" in a:
            try:
    ...
    if mode is None:
        ...
        LOGGER.warning(
            f"WARNING ⚠️ 'mode' is missing. Valid modes are {MODES}. Using default 'mode={mode}'."
        )
    ...
    if "rtdetr" in stem:  # guess architecture
        from yololab import RTDETR

        model = RTDETR(model)  # no task argument
    elif "fastsam" in stem:
        from yololab import FastSAM

        model = FastSAM(model)
    elif "sam" in stem:
        from yololab import SAM
    ...
    if mode in ("predict", "track") and "source" not in overrides:
        ...
        LOGGER.warning(
            f"WARNING ⚠️ 'source' is missing. Using default 'source={overrides['source']}'."
        )
    elif mode in ("train", "val"):
        if "data" not in overrides and "resume" not in overrides:
            overrides["data"] = DEFAULT_CFG.data
            LOGGER.warning(
                f"WARNING ⚠️ 'data' is missing. Using default 'data={overrides['data']}'."
            )
    elif mode == "export":
        if "format" not in overrides:
            overrides["format"] = DEFAULT_CFG.format or "torchscript"
            LOGGER.warning(
                f"WARNING ⚠️ 'format' is missing. Using default 'format={overrides['format']}'."
            )
