...
from yololab.utils import ARM64, LINUX, LOGGER, ROOT, yaml_load
...
class AutoBackend(nn.Module):
    """
    Handles dynamic backend selection for running inference using yololab YOLO models.
    ...
    def __init__(
        ...
        if nn_module:  # in-memory PyTorch model
            ...
            from yololab.nn.utils import attempt_load_weights
            ...
            from yololab.engine.exporter import gd_outputs
            ...
            from yololab.utils.triton import TritonRemoteModel

            model = TritonRemoteModel(w)
        else:
            from yololab.engine.exporter import export_formats

            raise TypeError(
                f"model='{w}' is not a supported model format. "
                "See https://docs.yololab.com/modes/predict for help."
                f"\n\n{export_formats()}"
            )
    ...
    def forward(self, im, augment=False, visualize=False, embed=None):
        ...
        if self.pt or self.nn_module:  # PyTorch
            ...
            if "confidence" in y:
                raise TypeError(
                    "yololab only supports inference of non-pipelined CoreML models exported with "
                    f"'nms=False', but 'model={w}' has an NMS pipeline created by an 'nms=True' export."
                )
                # TODO: CoreML NMS inference handling
                # from yololab.utils.ops import xywh2xyxy
            ...
            if self.saved_model:  # SavedModel
                ...
                for output in self.output_details:
                    ...
                    if x.ndim > 2:  # if task is not classification
                        # Denormalize xywh by image size. See https://github.com/yololab/yololab/pull/1695
